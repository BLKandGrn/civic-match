  ...
  return (
    <div ref={topRef}>
      ...
      <div className="screen-only" style={{ background:"#141414", border:"1px solid #222", borderRadius:"6px", padding:"22px", display:"flex", flexDirection:"column", gap:"14px", marginTop:"0" }}>
        ...
        {cur.heading === "Local Officials" && (
          <BlackVoteWatchTab members={props.federalMembers} zip={props.addr && props.addr.zip} mergeMode={true} />
        )}
        {cur.heading === "Local Elections" && (
          <BlackVoteWatchTab members={props.federalMembers} zip={props.addr && props.addr.zip} mergeMode={true} />
        )}
...
function BlackVoteWatchTab({ members, zip, mergeMode }) {
  ...
  if (mergeMode) {
    // Compact inline version shown inside Local Officials tab
    return (
      <div style={{ marginTop:"8px", borderTop:"1px solid #1e1e1e", paddingTop:"16px", display:"flex", flexDirection:"column", gap:"12px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          <div style={{ fontFamily:"Arial Black, Arial, sans-serif", fontWeight:900, fontSize:"13px", color:"#fff", letterSpacing:".5px" }}>BlackVoteWatch</div>
          <div style={{ fontSize:"11px", color:"#555" }}>— independent scoring of your federal reps on issues affecting Black Americans</div>
          <a href={BVW_BASE} target="_blank" rel="noopener noreferrer"
            style={{ fontFamily:"Arial, sans-serif", fontSize:"10px", color:GREEN, textDecoration:"none", border:"1px solid "+GREEN, borderRadius:"3px", padding:"2px 7px", letterSpacing:".04em", whiteSpace:"nowrap" }}>
            Visit Site
          </a>
        </div>
        {federalReps.length > 0 ? (
          <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
            {federalReps.map(function(rep, i) {
              ...
              return (
                <div key={i} style={{ background:"#111", border:"1px solid #222", borderRadius:"6px", padding:"10px 14px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"8px" }}>
                  <div>
                    <div style={{ fontSize:"13px", color:"#fff", fontWeight:700 }}>{rep.name}</div>
                  ...
                  {bvwUrl ? (
                    <a href={bvwUrl} target="_blank" rel="noopener noreferrer"
                      style={{ fontFamily:"Arial, sans-serif", fontWeight:700, fontSize:"11px", letterSpacing:".05em", color:"#0e0e0e", background:GREEN, borderRadius:"4px", padding:"6px 12px", textDecoration:"none", whiteSpace:"nowrap" }}>
                      View BVW Score
                    </a>
                  ) : (
                    <a href={BVW_BASE + "/rankings"} target="_blank" rel="noopener noreferrer"
                      style={{ fontFamily:"Arial, sans-serif", fontWeight:700, fontSize:"11px", letterSpacing:".05em", color:GREEN, border:"1px solid "+GREEN, borderRadius:"4px", padding:"6px 12px", textDecoration:"none", whiteSpace:"nowrap" }}>
                      Search Rankings
                    </a>
                  )}
                </div>
          ...
          <div style={{ fontSize:"12px", color:"#555" }}>Federal representatives not loaded — run a search to see BVW scores.</div>
        )}
        <div style={{ fontSize:"11px", color:"#444", lineHeight:1.6 }}>
          Grades based on voting records, co-sponsorship patterns, and donor analysis. Independent research by BlackVoteWatch — not affiliated with Civic Match or BLK + GRN.{" "}
          <a href={BVW_BASE + "/methodology"} target="_blank" rel="noopener noreferrer" style={{ color:GREEN, textDecoration:"underline" }}>Methodology</a>
        </div>
      </div>
    );
  }
  ...
  useEffect(function() {
    ...
    async function load() {
      try {
        ...
        const combined = [...(senData.results || []), ...(houseData.results || [])];
        // Deduplicate by candidate_id
        const seen = new Set();
        const all = combined.filter(function(c) {
